#!/usr/bin/env python3
"""Phase 2 v4: self-play with MCTS-improved policy targets (real AlphaZero)."""
import argparse
import os
import sys
...
import torch
# === Performance optimizations (TF32 + cudnn autotuner + threading) ===
...
from training.trainer_v4 import TrainerV4
...
def main():
    ...
    if args.resume and os.path.exists(args.resume):
        print(f"[Phase2-v4] Loading checkpoint: {args.resume}")
        sd = torch.load(args.resume, map_location=trainer.device, weights_only=True)
        trainer.network.load_state_dict(sd)
        trainer.ema = type(trainer.ema)(trainer.network)   # reset EMA from loaded weights
    ...
    print(f"\n[OK] Phase2-v4 done. Best mean_score={trainer.best_mean_score:.1f} "
          f"at iter={trainer.best_iter}")
    print(f"     Logs: {trainer.run_dir}")
    print(f"     Best: {args.checkpoint_dir}/phase2_best_v4.pt")
